...
class SHA256:
    ...
    #信息预处理。附加填充和附加长度值
    def Pad(self, W):
        return bytes.fromhex(W) + b"\x80" + (b"\x00" * ((55 if (len(W) % 64) < 56 else 119) - (len(W) % 64))) + (
...
class HMAC():
    ...
    def __init__(self, key, b, hash) -> None:
        self.ipad = int("".join(["00110110"] * (b // 8)),2)
        self.opad = int("".join(["01011100"] * (b // 8)),2)
        hex_key = hex(int(key, 16))[2:]
        if len(hex_key)<64:
            padding_length = 64-len(hex_key)
            key = int((hex_key + "0" * padding_length),16)
        else:
            key = int(hash.hash(hex_key),16)
        self.key = key
        self.encoder = hash
        
    def ascii2int(self, a):
        return binascii.hexlify(a.encode()).decode()
        
    def hash(self, plaintext):
        a = self.key^self.ipad
        
        combine_a = hex(a)[2:]  + self.ascii2int(plaintext)
        plaintext = self.encoder.hash(combine_a)
        b = self.key^self.opad
        combine_b = hex(b)[2:]  + plaintext
        plaintext = self.encoder.hash(combine_b)
        return plaintext 
